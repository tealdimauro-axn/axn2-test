<?php
/* Axn_fiserv Payment Gateway Class */

include_once('includes/axn-fiserv-utility.php');

class AXN_Axn_fiserv_Gateway extends WC_Payment_Gateway
{
    ...
    function __construct()
    {
        ...
        define('AXN_FISERV_TEMPLATE_PATH', untrailingslashit(plugin_dir_path(__FILE__)) . '/templates/');
        // The global ID for this Payment method
        $this->id = "axn_fiserv_gateway"; //axn_fiserv_india

        define('WC_AXN_FISERV_PLUGIN_URL', untrailingslashit(plugins_url(basename(plugin_dir_path(__FILE__)), basename(__FILE__))));
        ...
        add_action('woocommerce_api_axn_fiserv', array($this, 'webhook'));
        ...
        $this->instalment_details = get_option('woocommerce_axn_fiserv_multiple_payment', array(
                array(
    ...
    /**
     * Build the administration fields for Axn Fiserv IPG Gateway
     */
    public function init_form_fields()
    {
        $this->form_fields = include('includes/axn-fiserv-settings.php');
    }
    ...
    public function showInstallment()
    {
        ...
        if (count($applicable_installment_payment) > 0) {
            wc_get_template('multiple_payment_card.php', array('installment_payment' => $applicable_installment_payment, 'card_total' => $totalcartamount), 'axn-fiserv-gateway/', AXN_FISERV_TEMPLATE_PATH);
        }
    }
    ...
    {
        $multiple_payment = get_option('woocommerce_axn_fiserv_multiple_payment');
        return $multiple_payment;
    }

    /**
     * Generate the Axn_fiserv Payment Gateway button link.
     *
     * @since 1.0.0
     */
    function generate_ipg_in_form($order_id)
    {
        ...
        $successUrl = $customSuccessUrl != '' ? $customSuccessUrl : $order->get_checkout_order_received_url();
        $failedUrl = $customFailedUrl != '' ? $customFailedUrl : $order->get_cancel_order_url();
        ...
        if ($enable_installment == "multiplepayment") {
            ...
            $installment_details = get_option('woocommerce_axn_fiserv_multiple_payment');
        ...
        cuotas de <span style="color: #000000;"><strong>$' . round($valorcuota, 2) . '</strong></span> cada una.</p>
            ...
            if ($country == "mex") {
                if ($reseller == "axn_fiservmexico" && $this->interest_instalment == "yes") {
...
?>
    ...
    public function localPaymentMethod()
    {
        ...
        if ('yes' == $this->local_payment_method && $this->authorization == 'hidden') {
            wp_enqueue_script('axn_fiserv_gateway_script', plugins_url('assets/js/axn-fiserv-gateway.js', __FILE__), array('jquery'), WC()->version, true);

            $localSettingData = array('localPaySetting' => $this->paymentOptionsValue());
            wp_localize_script('axn_fiserv_gateway_script', 'fd_setting', $localSettingData);
        }

        wc_get_template('local-payment-method.php', array('local_payment' => $local_payment_method_options), 'axn-fiserv-gateway/', AXN_FISERV_TEMPLATE_PATH);
    }
...
?>
                ...
                <script type="text/javascript">
                    jQuery(function() {
                        jQuery('#fd_instalment').on( 'click', 'a.add', function(){ 
                            var country = jQuery('select#woocommerce_axn_fiserv_gateway_country').find('option:selected').val();
                            ...
                            if(size < 60){
                                first = '<tr class="instalment">\
                                            ...
                                            <td><input type="text" name="multipayment_maxamount[' + size + ']" /></td>';
                                                    ...
                                                    if (jQuery('#woocommerce_axn_fiserv_gateway_rule_activation').is(":checked")) {
    ...
    public function save_instalment_details()
    {
        ...
        if (isset($_POST['woocommerce_axn_fiserv_gateway_rule_activation']) &&
        ($_POST['woocommerce_axn_fiserv_gateway_rule_activation'] == 1) &&
        ...
        update_option('woocommerce_axn_fiserv_multiple_payment', $instalmentsDetails);
    }
    ...
    public function form()
    {
        ...
        $cvc_field = '<p class="form-row form-row-last">
			<label for="' . esc_attr($this->id) . '-card-cvc">' . esc_html__('Card code', 'woocommerce-axn_fiserv') . ' <span class="required">*</span></label>
        ...
        if ($cardtypecheck == 'yes') {
            $cardtype = '<p class="form-row form-row-wide">
				<label for="' . esc_attr($this->id) . '-card-type">' . esc_html__('Debit/Credit', 'woocommerce-axn_fiserv') . ' <span class="required">*</span></label>
                                <select id="' . esc_attr($this->id) . '-card-type" class="input-text wc-credit-card-form-card-type" ' . $this->field_name('card-type') . ' >
                                <option value="credit"> ' . esc_html__('Credit Card', 'woocommerce-axn_fiserv') . ' </option>         
                                <option value="debit">' . esc_html__('Debit Card', 'woocommerce-axn_fiserv') . '</option>                                          
                                </select> 
                        </p>';
        }
        else {
        ...
        $default_fields = array(
            ...
            'card-number-field' => '<p class="form-row form-row-wide">
				<label for="' . esc_attr($this->id) . '-card-number">' . esc_html__('Card number', 'woocommerce-axn_fiserv') . ' <span class="required">*</span></label>
				<input id="' . esc_attr($this->id) . '-card-number" class="input-text wc-credit-card-form-card-number" inputmode="numeric" autocomplete="cc-number" autocorrect="no" autocapitalize="no" spellcheck="no" type="tel" placeholder="&bull;&bull;&bull;&bull; &bull;&bull;&bull;&bull; &bull;&bull;&bull;&bull; &bull;&bull;&bull;&bull;" ' . $this->field_name('card-number') . ' />
			</p>',
            'card-expiry-field' => '<p class="form-row form-row-first">
				<label for="' . esc_attr($this->id) . '-card-expiry">' . esc_html__('Expiry (MM/YY)', 'woocommerce-axn_fiserv') . ' <span class="required">*</span></label>
...
?>
    ...
    public function payment_success($responseData)
    {
        ...
        if ($this->get_option('env') == "TEST" && $this->get_option('test_success_url') != '') {
            $responseSuccessURL = $this->get_option('test_success_url');
        }
        elseif ($this->get_option('env') != "TEST" && $this->get_option('prod_success_url') != '') {
            $responseSuccessURL = $this->get_option('prod_success_url');
        }
        else {
    ...
    public function payment_failure($responseData)
    {
        ...
        if ($this->get_option('env') == "TEST" && $this->get_option('test_fail_url') != '') {
            $failURL = $this->get_option('test_fail_url');
        }
        elseif ($this->get_option('env') != "TEST" && $this->get_option('prod_fail_url') != '') {
            $failURL = $this->get_option('prod_fail_url');
        }
        else {
...
?>
        ...
        <script type="text/javascript">
            jQuery(function() {
                ...
                var featuresSettings = eval( <?php echo json_encode(Axn_fiserv_Utility::getFeatures()); ?> );
                var countrySelected = '';
                ...
                jQuery('.country').change('click',function() {
                    ...
                    if(featuresSettings.hasOwnProperty(country)){
                        ...
                        jQuery('select[name="woocommerce_axn_fiserv_gateway_reseller"]').find('option').not(':first').remove();
                        jQuery('select[name="woocommerce_axn_fiserv_gateway_reseller"]').append( sellerOptions );
                    }
                    if(country == 'mex') {
                ...
                jQuery('.reseller').change('click',function() {
                     ...
                     * Added by Mom
                     */
                    if(resellerSelected == "axn_fiservmexico"){
                        jQuery("#woocommerce_axn_fiserv_gateway_interest_instalment").closest("tr").show();
                    }
                    else{
                        jQuery("#woocommerce_axn_fiserv_gateway_interest_instalment").closest("tr").hide();
                        jQuery("#woocommerce_axn_fiserv_gateway_interest_instalment").attr('checked', false);
                    }
                    /*
                    ...
                    if(localPayment.length != 0) {
                        ...
                        for (var paykey in localPayment) {
                            if (localPayment.hasOwnProperty(paykey)) {
                                ...
                                jQuery('select[name="woocommerce_axn_fiserv_gateway_local_payment_method_options[]"]').find('option').remove();
                                jQuery('select[name="woocommerce_axn_fiserv_gateway_local_payment_method_options[]"]').append( localPaymentOptions );
                            }
                        } 
         ...
         */
        $country = $this->get_option('country');
        $feature = Axn_fiserv_Utility::getFeatures();
        $reseller = $this->get_option('reseller');
...
?>
                        ...
                        if(featuresSettings.hasOwnProperty(countrySel)){
                            ...
                            jQuery('select[name="woocommerce_axn_fiserv_gateway_reseller"]').find('option').not(':first').remove();
                            jQuery('select[name="woocommerce_axn_fiserv_gateway_reseller"]').append( sellerOptions );
                            jQuery('select[name="woocommerce_axn_fiserv_gateway_reseller"] option[value="'+sellerSel+'"]').attr("selected",true);
...
?>
    ...
    public function process_admin_options()
    {
        ...
        if (array_key_exists('woocommerce_axn_fiserv_gateway_server_trust_pem', $_FILES)
        && array_key_exists('tmp_name', $_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem'])
        && array_key_exists('size', $_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem'])
        && $_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem']['size'] && $this->file_validate($_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem']['tmp_name'], "text/plain")) {

            file_put_contents(dirname(__FILE__) . "/keys/ipg_online_trust.pem", file_get_contents($_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem']['tmp_name']));
            $_POST['woocommerce_axn_fiserv_gateway_server_trust_pem'] = str_replace('\\', '/', realpath(dirname(__FILE__)) . "/keys/ipg_online_trust.pem");
            unlink($_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem']['tmp_name']);
            unset($_FILES['woocommerce_axn_fiserv_gateway_server_trust_pem']);
        }
        else {
            $_POST['woocommerce_axn_fiserv_gateway_server_trust_pem'] = $this->get_option('server_trust_pem');
        }

        if (array_key_exists('woocommerce_axn_fiserv_gateway_client_certificate_pemfile', $_FILES)
        && array_key_exists('tmp_name', $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile'])
        && array_key_exists('size', $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile'])
        && $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile']['size'] && $this->file_validate($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile']['tmp_name'], "text/plain")) {

            file_put_contents(dirname(__FILE__) . "/keys/client_certificate.pem", file_get_contents($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile']['tmp_name']));
            $_POST['woocommerce_axn_fiserv_gateway_client_certificate_pemfile'] = str_replace('\\', '/', realpath(dirname(__FILE__)) . "/keys/client_certificate.pem");
            unlink($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile']['tmp_name']);
            unset($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_pemfile']);
        }
        else {
            $_POST['woocommerce_axn_fiserv_gateway_client_certificate_pemfile'] = $this->get_option('client_certificate_pemfile');
        }

        if (array_key_exists('woocommerce_axn_fiserv_gateway_client_certificate_keyfile', $_FILES)
        && array_key_exists('tmp_name', $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile'])
        && array_key_exists('size', $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile'])
        && $_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile']['size'] && $this->file_validate($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile']['tmp_name'], "text/plain")) {

            file_put_contents(dirname(__FILE__) . "/keys/client_certificate.key", file_get_contents($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile']['tmp_name']));
            $_POST['woocommerce_axn_fiserv_gateway_client_certificate_keyfile'] = str_replace('\\', '/', realpath(dirname(__FILE__)) . "/keys/client_certificate.key");
            unlink($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile']['tmp_name']);
            unset($_FILES['woocommerce_axn_fiserv_gateway_client_certificate_keyfile']);
        }
        else {
            $_POST['woocommerce_axn_fiserv_gateway_client_certificate_keyfile'] = $this->get_option('client_certificate_keyfile');
        }
    ...
    /**
     * Can the order be refunded via Axn Fiserv Gateway?
    ...
    {
        include_once(dirname(__FILE__) . '/includes/axn-fiserv-api-handler.php');

        Axn_fiserv_API_Handler::$api_url = $url = $this->getDetails('apiurl');
        Axn_fiserv_API_Handler::$api_username = $this->get_option('api_username');
        Axn_fiserv_API_Handler::$api_password = $this->get_option('api_password');
        Axn_fiserv_API_Handler::$certificate_key_password = $this->get_option('certificate_key_password');
        Axn_fiserv_API_Handler::$server_trust_pem = $this->get_option('server_trust_pem');
        Axn_fiserv_API_Handler::$client_certificate_pemfile = $this->get_option('client_certificate_pemfile');
        Axn_fiserv_API_Handler::$client_certificate_keyfile = $this->get_option('client_certificate_keyfile');
    }
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        $result = Axn_fiserv_API_Handler::refund_transaction($request_param);
    ...
    public function checkout_card_validation()
    {
        ...
        if ($_POST['payment_method'] == 'axn_fiserv_gateway') {
            ...
            $this->getCardSupportLocalPay($paymentMethod) == true) {
                ...
                if ($cardNumber == '') {
                    wc_add_notice(__('Card Number is a required field.', 'woocommerce-axn_fiserv'), 'error');
                }
                else if (!preg_match('/^[0-9]{14,16}$/', $cardNumber)) {
                    wc_add_notice(__('Card Numberis is Invalid.', 'woocommerce-axn_fiserv'), 'error');
                }


                if ($cardExpiry == '') {
                    wc_add_notice(__('Expiry Date is a required field.', 'woocommerce-axn_fiserv'), 'error');
                }
                else if (!preg_match('/^([0-9]{1,2})\/([0-9]{2}|[0-9]{4})$/', $cardExpiry) || !$expiry_validation) {
                    wc_add_notice(__('Expiry Date is Invalid.', 'woocommerce-axn_fiserv'), 'error');
                }

                if ($cardcvc == '') {
                    wc_add_notice(__('Card CVC is a required field.', 'woocommerce-axn_fiserv'), 'error');
                }
                else if (!preg_match('/^[0-9]{3,4}$/', $cardcvc)) {
                    wc_add_notice(__('Card CVC is Invalid.', 'woocommerce-axn_fiserv'), 'error');
                }
            }
        }
    }
    ...
    public static function log($message, $level = 'info')
    {
        if (self::$log_enabled) {
            ...
            self::$log->log($level, $message, array('source' => 'axn_fiserv'));
        }
    }
    ...
    {
        $return = $this->get_option('description') != '' ? $this->get_option('description') : 'Axn Fiserv Gateway extension for WooCommerce';
        return __($return, $this->id);
    }
